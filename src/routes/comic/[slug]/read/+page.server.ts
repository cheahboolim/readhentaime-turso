...
export async function load({ params, url: _url }) {
	...
	if (randomMangaIds.length > 0) {
		...
		if (!randomSlugError && randomSlugs) {
			...
			randomComics = finalRandomManga.map(
	...
	const seoDescription =
		pageNum === 1
			? `Read ${baseTitle} manga online for free at Read Hentai. High quality translated manga with fast updates.${tagNames.length > 0 ? ` Available genres: ${tagNames.slice(0, 3).join(', ')}.` : ''}`
			: `Continue reading ${baseTitle} - Page ${pageNum} at Read Hentai. Free online manga reader with high quality images and fast loading.`

	// Canonical URL
	const canonical = `https://readhentai.me/comic/${slug}/read${
	...
	const next = pageNum < totalPages ? `/comic/${slug}/read?page=${pageNum + 1}` : undefined
	...
	const jsonLd = {
		...
		url: `https://readhentai.me/comic/${slug}`,
		image: ogImage,
		genre: tagNames,
