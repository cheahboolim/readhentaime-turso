...
export const load: PageServerLoad = async ({ url, locals }) => {
	...
	try {
		...
		const searchPromises = [
			...
			// 2. Search by tags
			supabase.from('tags').select('manga_tags(manga_id)').ilike('name', `%${query}%`),

			// 3. Search by artists
			supabase.from('artists').select('manga_artists(manga_id)').ilike('name', `%${query}%`),

			// 4. Search by categories
			supabase.from('categories').select('manga_categories(manga_id)').ilike('name', `%${query}%`),

			// 5. Search by characters
			supabase.from('characters').select('manga_characters(manga_id)').ilike('name', `%${query}%`),

			// 6. Search by parodies
			supabase.from('parodies').select('manga_parodies(manga_id)').ilike('name', `%${query}%`),

			// 7. Search by groups
			supabase.from('groups').select('manga_groups(manga_id)').ilike('name', `%${query}%`)
		]
		...
		const sortedManga = paginatedMangaIds.map((id) => mangaMap.get(id)).filter(Boolean)
		...
		const meta = {
			title:
				page > 1
					? `Search results for "${query}" – Page ${page} | readhentai.me`
					: `Search results for "${query}" | readhentai.me`,
