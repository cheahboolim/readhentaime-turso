...
<svelte:head>
	...
	<meta name="author" content={comic.seoData.primaryArtist || 'NHentai Pics'} />
						...
						<div class="bg-pink-50 dark:bg-pink-900/20 rounded-lg p-4 mb-4">
							...
							{#if comic.seoData.topParody}
								<div>
									...
									<span class="text-pink-600 dark:text-pink-400 font-medium">{comic.seoData.topParody}</span>
								</div>
							{/if}
						</div>
					{/if}
		...
		<section class="bg-gray-50 dark:bg-gray-800 rounded-lg p-6">
			...
			<div class="prose prose-sm max-w-none text-gray-700 dark:text-gray-300">
				...
				{#if comic.seoData.topTags.length > 0}
					...
					<p class="mb-4">
						This manga explores themes of <strong>{comic.seoData.topTags.slice(0, 3).join(', ')}</strong>
				...
				<p>
					Read <strong>{comic.title}</strong> online for free with our mobile-friendly reader. 
					High-resolution images ensure the best viewing experience across all devices. 
					No registration required - start reading immediately!
				</p>
			</div>
		</section>
