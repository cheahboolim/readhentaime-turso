...
export const load: PageServerLoad = async ({ locals, url }) => {
  ...
  if (error || !groups) {
    ...
    return {
      ...
      availableLetters: [],
      seo: {
        title: 'Browse Hentai Groups | NHentai',
  ...
  return {
    ...
    availableLetters,
    seo: {
      title: `Browse ${totalGroups}+ Hentai Groups A-Z | NHentai`,
      ...
      ogImage: 'https://nhentai.pics/images/groups-og.jpg',
      structuredData: {
        ...
        mainEntity: {
          ...
          itemListElement: groups.slice(0, 25).map((group, index) => ({
            ...
            item: {
              '@type': 'Organization',
              name: group.name,
              url: `https://nhentai.pics/browse/groups/${group.slug}`,
              inDefinedTermSet: {
                '@type': 'DefinedTermSet',
                name: 'Hentai Groups'
              }
