...
export const load: PageServerLoad = async ({ params, url }) => {
	...
	const [
		...
		supabase.from('manga').select('id, title, feature_image_url').in('id', mangaIds),
		supabase.from('slug_map').select('slug, manga_id').in('manga_id', mangaIds),
		supabase
			.from('manga_artists')
	...
	const relatedDataByMangaId = mangaIds.reduce(
		(acc, mangaId) => {
			acc[mangaId] = {
				artists: (allArtists || [])
	...
	const topCharacters = comics
		.map((c) => c.seoData.characters)
		.flat()
		.filter(Boolean)
	const topTags = comics
		.map((c) => c.seoData.tags)
		.flat()
		.filter(Boolean)
	const topParodies = comics
		.map((c) => c.seoData.parodies)
		.flat()
		.filter(Boolean)
			...
			structuredData: {
				...
				mainEntity: {
					...
					itemListElement: comics.map((comic, index) => ({
						...
						item: {
							'@type': 'Book',
							'@id': `https://readhentai.me/hentai/${comic.slug}`,
							name: comic.title,
							url: `https://readhentai.me/hentai/${comic.slug}`,
				...
				breadcrumb: {
					...
					itemListElement: [
						...
						},
						{
							...
							position: 3,
							name: typeLabel + 's',
							item: `https://readhentai.me/p/${type}`
						},
						{
							'@type': 'ListItem',
							position: 4,
