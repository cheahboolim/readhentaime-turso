...
const defaultSEO: SEOData = {
	title: 'Read Hentai Pics | Premium Adult Manga & Doujinshi Online',
	description:
		'Read premium adult manga, doujinshi and hentai comics online free. Updated daily with high-quality translated content at readhentai.me',
	ogType: 'website',
...
export const seoHelpers = {
	...
	homepage: (page = 1) => ({
		title:
			page === 1
				? 'Read Hentai Pics | Read Premium Adult Manga & Doujinshi Free Online'
				: `Premium Adult Manga | Page ${page} | Read Hentai Pics`,
		description:
			page === 1
				? 'Discover thousands of premium adult manga, doujinshi and hentai comics. Updated daily with high-quality translated content. Read free online at readhentai.me'
				: `Browse page ${page} of premium adult manga. Thousands of high-quality doujinshi and hentai comics available free at readhentai.me`,
		canonical: page === 1 ? 'https://readhentai.me' : `https://readhentai.me?page=${page}`,
		keywords:
			'adult manga, doujinshi, hentai comics, free manga online, translated manga, premium adult content',
		ogImage: '/images/Read Hentai-home-og.jpg'
	}),
	...
	manga: (comic: any, slug: string) => {
		...
		return {
			title: `${comic.title} | Read Free Adult Manga | Read Hentai Pics`,
			description: `Read ${comic.title} online free. ${tags.length > 0 ? `Tagged with ${tags.slice(0, 3).join(', ')}.` : ''} ${artists.length > 0 ? `By ${artists[0]}.` : ''} High quality adult manga at readhentai.me`,
			canonical: `https://readhentai.me/manga/${slug}`,
			...
			jsonLd: {
				...
				url: `https://readhentai.me/manga/${slug}`,
				publisher: {
					'@type': 'Organization',
	...
	reader: (manga: any, slug: string, currentPage: number, totalPages: number) => ({
		title: `${manga.title} - Page ${currentPage} | Online Manga Reader | Read Hentai Pics`,
		description: `Read ${manga.title} page ${currentPage} of ${totalPages} online. Free manga reader with high quality images and fast loading at readhentai.me`,
		canonical: `https://readhentai.me/manga/${slug}/read/${currentPage}`,
		...
		jsonLd: {
			...
			url: `https://readhentai.me/manga/${slug}/read/${currentPage}`,
			isPartOf: {
				'@type': 'Book',
				name: manga.title,
				url: `https://readhentai.me/manga/${slug}`
	...
	browse: (type: string, name: string, slug: string, page: number, totalManga: number) => {
		...
		return {
			title: `${name} ${label} - ${totalManga} Manga${pageInfo} | Read Hentai Pics`,
			description: `Browse ${totalManga} adult manga with ${name} ${label.toLowerCase()}${pageInfo}. High-quality doujinshi and hentai comics at readhentai.me`,
			canonical:
				page === 1
			...
			prev:
				page > 1 ? `/browse/${type}/${slug}${page === 2 ? '' : `?page=${page - 1}`}` : undefined,
			next:
				page < Math.ceil(totalManga / 10) ? `/browse/${type}/${slug}?page=${page + 1}` : undefined
		}
	},
	...
	search: (query: string, page: number, totalResults: number) => ({
		title:
			page === 1
				? `Search: "${query}" | Read Hentai Pics`
				: `Search: "${query}" - Page ${page} | Read Hentai Pics`,
		description: `Search results for "${query}" on Read Hentai Pics. Found ${totalResults} adult manga and doujinshi${page > 1 ? ` - Page ${page}` : ''}.`,
		canonical:
			page === 1
				? `https://readhentai.me/search?q=${encodeURIComponent(query)}`
				: `https://readhentai.me/search?q=${encodeURIComponent(query)}&page=${page}`,
		keywords: `${query}, search, adult manga, doujinshi, hentai comics`,
		robots: totalResults > 0 ? 'index, follow' : 'noindex, follow',
		prev:
			page > 1
				? `/search?q=${encodeURIComponent(query)}${page === 2 ? '' : `&page=${page - 1}`}`
				: undefined,
		next:
	...
	random: (page: number) => ({
		title:
			page === 1
				? 'Random Adult Manga | Discover New Comics | Read Hentai Pics'
				: `Random Adult Manga - Page ${page} | Read Hentai Pics`,
		description:
			'Discover random adult manga and doujinshi. Find new comics you might have missed with our random selection at readhentai.me',
		canonical:
			page === 1 ? 'https://readhentai.me/random' : `https://readhentai.me/random?page=${page}`,
