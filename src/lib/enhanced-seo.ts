...
export class SEOManager {
  static generateMangaPageSEO(
    ...
    const description = page === 1
      ? `Read ${baseTitle} manga online for free at NHentai. High quality translated manga with fast updates.${manga.tagNames?.length ? ` Available genres: ${manga.tagNames.slice(0, 3).join(', ')}.` : ''}`
      : `Continue reading ${baseTitle} - Page ${page} at NHentai. Free online manga reader with high quality images and fast loading.`;

    const canonical = `https://nhentai.pics/hentai/${slug}/${page}`;
  ...
  static generateBrowsePageSEO(
    ...
    const title = page === 1 
      ? `${name} ${typeLabel} - ${totalManga} Manga | NHentai`
      : `${name} ${typeLabel} - Page ${page} of ${totalPages} | NHentai`;
    ...
    return {
      ...
      ogDescription: description,
      ogType: 'website',
      prev: page > 1 ? (page === 2 ? `/browse/${type}/${slug}` : `/browse/${type}/${slug}?page=${page - 1}`) : undefined,
  ...
  private static generateBrowseDescription(type: string, name: string, page: number, totalManga: number): string {
    const pageInfo = page > 1 ? ` - Page ${page}` : '';
